...
class Button implements Drawable, Sensible {
  ...
  private indicateTimeRemaining: boolean = false;
  private indicateSpinning: boolean = true;
  ...
  draw(
    ...
    const isDisabled =
      !this.skipDisabledCheck &&
      (gameState.spinRequested ||
  ...
  checkSensors(screenContext: ScreenContext): void {
    ...
    if (screenContext.events.mouse.down) {
      ...
      if (
        mouseEvent.x >= localShiftX &&
        mouseEvent.x <= localShiftX + width &&
        mouseEvent.y >= localShiftY &&
        mouseEvent.y <= localShiftY + height
      ) {
        this.eventListener();
      }
    }
  ...
  setIndicateTimeRemaining(indicate: boolean) {
    this.indicateTimeRemaining = indicate;
  }

  setIndicateSpinning(indicate: boolean) {
    this.indicateSpinning = indicate;
  }
}
